...
def _make_params_mobileone2d_basic(
    kernel_size_list: list[int], num_conv_branches_list: list[int]
) -> list[dict[str, Any]]:
    res = []
    ...
    for ncb in num_conv_branches_list:
...
TEST_PARAMS_MOBILEONE2D_BASIC = _make_params_mobileone2d_basic(
    kernel_size_list=[1, 3, 5, 7],
    num_conv_branches_list=[1, 2, 5],
)


@pytest.mark.parametrize("params", TEST_PARAMS_MOBILEONE2D_BASIC)
def test_mobileone2d_forward(params: dict[str, Any]) -> None:
    m = ptrepa.MobileOneConv2d(**params)
...
@pytest.mark.parametrize("params", TEST_PARAMS_MOBILEONE2D_BASIC)
def test_fuse_mobileone2d(params: dict[str, Any]) -> None:
    # Original
    ...
    m_fused = ptrepa.make_conv2d_from_mobileone2d(m)
...
# #     m = ptrepa.RepVGG2d(**test_params)
# #     x = torch.rand(BATCH_SIZE, test_params["in_channels"], 224, 224)
# #     m.eval()
# #     with torch.inference_mode():
# #         y = m(x)
...
#     x = torch.rand(BATCH_SIZE, params["in_channels"], 224, 224)
#     m.eval()
#     with torch.inference_mode():
#         y = m(x)
...
#     {
#         "module": ptrepa.RepVGG2d(in_channels=3, out_channels=6, kernel_size=3),
#         "is_repvgg2d": True,
#     },
#     {
#         "module": ptrepa.RepVGG2d(in_channels=3, out_channels=3, kernel_size=3),
#         "is_repvgg2d": True,
#     },
#     {
...
#             ptrepa.RepVGG2d(in_channels=3, out_channels=6, kernel_size=3)
#         ),
#         "is_repvgg2d": True,
#     },
#     {
#         "module": torch.fx.symbolic_trace(
#             ptrepa.RepVGG2d(in_channels=3, out_channels=3, kernel_size=3)
#         ),
#         "is_repvgg2d": True,
#     },
# ]
...
# def _mask_channels(x: torch.Tensor, channel_mask: torch.Tensor) -> torch.Tensor:
...
#             output_mask[i] = True
#         elif params["groups"] == params["in_channels"]:
...
#         msg = f"Distill in_channels={in_channels} -> {predecessor_mask.sum().item()}"
#         logger.info(msg)
#         msg = f"Distill out_channels={out_channels} -> {output_mask.sum().item()}"
#         logger.info(msg)
...
#         x_masked = _mask_channels(x, predecessor_mask)
...
#             for t in templates:
#                 res.append({**t, **{"kernel_size": ks,
#                               "identity_conv": identity_conv}})
#     return res


# TEST_PARAMS_REPVGG2D_DISTILL = _make_params_repvgg2d_distill([3, 5, 7])
