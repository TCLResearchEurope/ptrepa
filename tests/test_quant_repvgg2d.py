...
def _make_params_quant_repvgg2d_basic(
...
TEST_PARAMS_QUANT_REPVGG2D_BASIC = _make_params_quant_repvgg2d_basic([1, 3, 5, 7])


@pytest.mark.parametrize("params", TEST_PARAMS_QUANT_REPVGG2D_BASIC)
def test_quat_repvgg2d_properties(params: dict[str, Any]) -> None:
    m = ptrepa.QuantRepVGGConv2d(**params)
    ...
    if m.in_channels == m.out_channels:
        if params["identity_conv"] == "trainable":
            assert m.qrvg_identity_conv is not None
            assert m.qrvg_identity_conv.weight.requires_grad is True
        elif params["identity_conv"] == "fixed":
            assert m.qrvg_identity_conv is not None
            assert m.qrvg_identity_conv.weight.requires_grad is False
        elif params["identity_conv"] == "disable":
            assert m.qrvg_identity_conv is None


@pytest.mark.parametrize("params", TEST_PARAMS_QUANT_REPVGG2D_BASIC)
def test_quant_repvgg2d_forward(params: dict[str, Any]) -> None:
    m = ptrepa.QuantRepVGGConv2d(**params)
...
@pytest.mark.parametrize("params", TEST_PARAMS_QUANT_REPVGG2D_BASIC)
def test_fuse_quant_repvgg2d(params: dict[str, Any]) -> None:
    # Original
    ...
    m_fused = ptrepa.make_conv2d_from_quant_repvgg2d(m)
...
@pytest.mark.parametrize("params", TEST_PARAMS_QUANT_REPVGG2D_BASIC)
def test_make_quant_repvgg_from_conv2d(params: dict[str, Any]) -> None:
    ...
    m_repa = ptrepa.make_quant_repvgg2d_from_conv2d(m)
    m_repa.eval()
    with torch.inference_mode():
        y_repa = m_repa(x)
...
TEST_PARAMS_IS_QUANT_REPVGG2D = [
    {
        "module": ptrepa.QuantRepVGGConv2d(
            in_channels=3, out_channels=6, kernel_size=3
        ),
        "is_quant_repvgg2d": True,
    },
    {
        "module": ptrepa.QuantRepVGGConv2d(
    ...
    },
    {
        ...
        "is_quant_repvgg2d": False,
    },
    {
        "module": torch.fx.symbolic_trace(
            ptrepa.QuantRepVGGConv2d(in_channels=3, out_channels=6, kernel_size=3)
        ),
        "is_quant_repvgg2d": True,
    },
    {
        "module": torch.fx.symbolic_trace(
            ptrepa.QuantRepVGGConv2d(in_channels=3, out_channels=3, kernel_size=3)
        ),
        "is_quant_repvgg2d": True,
    },
]
...
@pytest.mark.parametrize("params", TEST_PARAMS_IS_QUANT_REPVGG2D)
def test_is_quant_repvgg2(params: dict[str, Any]) -> None:
    m = params["module"]
    is_quant_repvgg2d = params["is_quant_repvgg2d"]
    assert ptrepa.is_quant_repvgg2d(m) == is_quant_repvgg2d


def check_distill_quant_repvgg2d_in_place(
    params: dict[str, Any],
    device: torch.device,
    float64: bool = False,
    ...
    for _ in range(5):
        ...
        m = ptrepa.QuantRepVGGConv2d(
            in_channels=in_channels,
            out_channels=out_channels,
            groups=groups,
            kernel_size=kernel_size,
        )
        logger.info(f"{m.qrvg_identity_conv=}")
        m.to(device)
        if float64:
        ...
        ptrepa.distill_quant_repvgg2d_in_place(
            m, predecessor_mask=predecessor_mask, output_mask=output_mask
        )
        assert m.qrvg_dense_conv.weight.shape[0] == out_channels_new
        assert m.groups != 1 or m.qrvg_dense_conv.weight.shape[1] == in_channels_new
        ...
        m_pruned_fused = ptrepa.make_conv2d_from_quant_repvgg2d(m)
...
def _make_params_quant_repvgg2d_distill(
...
TEST_PARAMS_QUANT_REPVGG2D_DISTILL = _make_params_quant_repvgg2d_distill([1, 3, 5, 7])


@pytest.mark.parametrize("params", TEST_PARAMS_QUANT_REPVGG2D_DISTILL)
def test_distill_quant_repvgg2d_in_place_cpu(params: dict[str, Any]) -> None:
    logger.info(params)
    check_distill_quant_repvgg2d_in_place(params, torch.device("cpu"))


@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires cuda")
@pytest.mark.parametrize("params", TEST_PARAMS_QUANT_REPVGG2D_DISTILL)
def test_distill_quant_repvgg2d_in_place_cuda(params: dict[str, Any]) -> None:
    logger.info(params)
    # During the tests floating point error seems to acumulate, hence degraded *tol
    check_distill_quant_repvgg2d_in_place(
        params, torch.device("cuda"), atol=0.0009, rtol=1.0e-6
    )


@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires cuda")
@pytest.mark.parametrize("params", TEST_PARAMS_QUANT_REPVGG2D_DISTILL)
def test_distill_quant_repvgg2d_in_place_cuda64(params: dict[str, Any]) -> None:
    # During the tests floating point error seems to acumulate
    # float64 test ensure that this is only
    logger.info(params)
    check_distill_quant_repvgg2d_in_place(params, torch.device("cuda"), float64=True)
